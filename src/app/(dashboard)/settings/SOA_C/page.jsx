...
const SOA_C = () => {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
        ...
        setSoas((prev) =>
            prev.map((soa) => (soa.soaId === id ? { ...soa, implementationStatus: newStatus } : soa))
        );
  ...
  return (
    <Box sx={{ padding: "20px", maxWidth: "900px", margin: "0 auto" }}>
      <ToastContainer />
      <Typography variant="h4" align="center">SOA Management</Typography>
      ...
      {loading ? (
        ...
        soas.map((soa) => (
          <Box key={soa._id} sx={{ margin: "10px 0" }}>
            <Typography>{soa.controlId} - {soa.controlName}</Typography>
            <Select
              value={soa.implementationStatus}
              onChange={(e) => handleStatusChange(soa.soaId, e.target.value)}
            >
